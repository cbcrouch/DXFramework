...
unsigned __stdcall renderThread(void *pArgs)
{
    ...
    while(running) {
        ...
        pRenderer->pImmediateContext->UpdateSubresource(g_hlslConstants.pChangesEveryFrame, 0, nullptr, &cb, 0, 0);

        // vertex shader setup
        pRenderer->pImmediateContext->VSSetShader(g_hlslProgram.pVertexShader, nullptr, 0);
        ...
        // pixel shader setup
        pRenderer->pImmediateContext->PSSetShader(g_hlslProgram.pPixelShader, nullptr, 0);
...
)
{
    ...
    if (mmr != MMSYSERR_NOERROR) {
        ...
        //
        // TODO: find out why timGetDevCaps is failing
        //
        //DXF_ERROR_BOX(); // no error reporting
    }
    ...
    assert(dxRenderer.pDevice != nullptr);
    ...
    // start render thread
    hRenderThread = (HANDLE)_beginthreadex(nullptr, 0, renderThread, (void *)&dxRenderer, 0, &threadId);
    if (!hRenderThread) {
        return -1;
    }
